...
<head>
    <meta charset="utf-8">
    <title>글로벌 네트워크 &lt; Company &lt; SK Networks</title>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            if(_width < 1024) {
                mapToggleButtons.forEach(button => {
                    button.setAttribute('title', '구글맵으로 새창 이동');
                })
            } else {
                mapToggleButtons.forEach(button => {
                    button.addEventListener('click', () => {
                        // 현재 title 값에 따라 title 값을 토글
                        if (button.getAttribute('title') === '열기') {
        ...
        <div class="main-inner">
            <div class="local case2">
                <div class="local-head design5" data-local-animation="case-2">
                    <div class="local-info">
                        <ul class="data-list breadcrumb design1 case2 dark">
                            ...
                            <li class="data-item"><span class="item-text">글로벌 네트워크</span></li>
                        </ul>
                    </div>
                    <div class="local-subject">
                        <h2 class="local-name">WE NETWORK<br>함께 만들어가는 더 큰 행복</h2>
                    </div>
                    <div class="local-aside">
                        <div class="local-wrap">
                            <h3 class="local-title">글로벌 네트워크</h3>
                        </div>
                    </div>
                ...
                <div class="local-body">
                    <div class="content-head blind">글로벌 네트워크 페이지</div>
                                                        ...
                                                        <div class="board-item">
                                                            <div class="board-wrap">
                                                                <div class="board-head" role="button" tabindex="0">
                                                                    <strong class="board-subject" style="word-break: break-all;">${item.companyNameKo}</strong>
                                                                </div>
                                                                <div class="board-body">
                                                                    <div class="board-inner">
                                                                        <div class="info-list">
                                                                            <div class="info-item">
                                                                                <strong class="info-title">국가</strong>
                                                                                <span class="info-content">${item.countryNameKo}</span>
                                                                            </div>
                                                                            <c:if test="${not empty item.addressKo}">
                                                                                <div class="info-item">
                                                                                    <strong class="info-title">주소</strong>
                                                                                    <span class="info-content" style="word-break: break-all;">${item.addressKo}</span>
                                                                                </div>
                                                                            </c:if>
                                                                            <c:if test="${not empty item.phoneNoKo}">
                                                                                <div class="info-item">
                                                                                    <strong class="info-title">전화번호</strong>
                                                                                    <span class="info-content"><c:out value="${item.phoneNoKo}"/></span>
                                                                                </div>
                                                                            </c:if>
                                                                        </div>
                                                                        <div class="btn-display design1 case1">
                                                                            <div class="btn-area">
                                                                                <button class="btn design1 case1 type1 color1 map" title="열기" data-latitude="${item.latitude}" data-longitude="${item.longitude}"><span class="btn-text">MAP</span></button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <div class="board-list">
                                                            <c:forEach items="${items}" var="item" varStatus="status">
                                                                <div class="board-item">
                                                                    <div class="board-wrap">
                                                                        <div class="board-head" role="button" tabindex="0">
                                                                            <strong class="board-subject"><c:out value="${item.companyNameKo}"/></strong>
                                                                        </div>
                                                                        <div class="board-body">
                                                                            <div class="board-inner">
                                                                                <div class="info-list">
                                                                                    <div class="info-item">
                                                                                        <strong class="info-title">국가</strong>
                                                                                        <span class="info-content"><c:out value="${item.countryNameKo}"/></span>
                                                                                    </div>
                                                                                    <c:if test="${not empty item.addressKo}">
                                                                                        <div class="info-item">
                                                                                            <strong class="info-title">주소</strong>
                                                                                            <span class="info-content"
                                                                                                  style="word-break: break-all;">${item.addressKo}</span></div>
                                                                                    </c:if>
                                                                                    <c:if test="${not empty item.phoneNoKo}">
                                                                                        <div class="info-item">
                                                                                            <strong class="info-title">전화번호</strong>
                                                                                            <span class="info-content">${item.phoneNoKo}</span>
                                                                                        </div>
                                                                                    </c:if>
                                                                                </div>
                                                                                <div class="btn-display design1 case1">
